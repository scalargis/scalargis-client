...
const getValidColumnProps = (scopedSchema: JsonSchema) => {
  if (
    ...
    return Object.keys(scopedSchema.properties).filter(
      ...
      (prop) => scopedSchema && scopedSchema.properties && scopedSchema.properties[prop].type !== 'array'
    );
  }
  // primitives
  return [''];
...
const ctxToNonEmptyCellProps = (
  ...
  const errors = formatErrorMessage(
    union(
      ...
      )((ctx?.core?.errors && ctx.core.errors) || []).map((error: any) => error.message)
    )
  );
  return {
...
const NonEmptyRowComponent = ({
  ...
  const moveUp = useMemo(
    ...
    () =>  moveUpCreator && moveUpCreator(path, rowIndex),
    [moveUpCreator, path, rowIndex]
  );
  const moveDown = useMemo(
    //() => moveDownCreator(path, rowIndex),
    () => moveDownCreator && moveDownCreator(path, rowIndex),    
    [moveDownCreator, path, rowIndex]
  );
  //TODO
  return (
    <div>TODO TableRow</div>
  );
  /*
  return (
