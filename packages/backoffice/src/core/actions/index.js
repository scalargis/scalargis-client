...
export const BACKOFFICE_SET_VIEWERSFILTER = 'BACKOFFICE_SET_VIEWERSFILTER'
/*
...
export function backoffice_set_viewersfilter(filter) {
  const action = {
    type: BACKOFFICE_SET_VIEWERSFILTER,
    filter
  }
  return action;
}
...
export function backoffice_load(core, history) {
  return function (dispatch, getState) {
    ...
    // Fetch backoffice
    let url = CONFIG_URL ? CONFIG_URL : API_URL + '/api/v2/app/backoffice/' + (history.location.search || '');
