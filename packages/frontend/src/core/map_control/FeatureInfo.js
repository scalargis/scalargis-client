...
export default function FeatureInfo({ config, actions }) {
  ...
  function searchAll(coordinate, pixel) {
    ...
    const lrs = findActiveWMSLayer(mainMap.getLayers());
    lrs.forEach(l => {
      if (l.get('selectable') === false) return;
      ...
      // Get features using any other strategy
        ...
        parseFeatureInfo(l, viewResolution, coordinate, mapCRS, (result) => {
...
const getWFSFeaturesFilterByGeom = (serviceUrl, typeNames, srsName, count, bbox, cql_filter) => {
  let params = [
    "SERVICE=wfs",
    "REQUEST=GetFeature",
    "VERSION=1.1.0",
...
const getFeatureInfoGeoJSON = (l, viewResolution, coordinate, crs, requestBuilder, resolve, reject) => {
  let url = requestBuilder(l, viewResolution, coordinate, crs, 'application/geojson');
  fetch(url)
  .then(res => res.json())
  .then(geojson => {
...
const getFeatureInfoJSON = (l, viewResolution, coordinate, crs, requestBuilder, resolve, reject) => {
  let url = requestBuilder(l, viewResolution, coordinate, crs, 'application/json'); 
  fetch(url)
  .then(res => res.json())
  .then(json => {
...
const getFeatureInfoXML = (l, viewResolution, coordinate, crs, requestBuilder, resolve, reject) => {
  let url = requestBuilder(l, viewResolution, coordinate, crs, 'text/xml');  
  fetch(url)
  .then(res => res.text())
  .then(xml => {
...
const getFeatureInfoGML = (l, viewResolution, coordinate, crs, format, requestBuilder, resolve, reject) => {
  let layerProps = l.getProperties();
  let url = requestBuilder(l, viewResolution, coordinate, crs, format || 'application/vnd.ogc.gml');
  fetch(url)
  .then(res => res.text())
  .then(gmlt => {
...
const getFeatureInfoHtml = (l, viewResolution, coordinate, crs, requestBuilder, resolve, reject) => {
  let url = requestBuilder(l, viewResolution, coordinate, crs, 'text/html');  
  fetch(url)
  .then(res => res.text())
  .then(text => {
    resolve(text);
...
const getFeatureInfoText = (l, viewResolution, coordinate, crs, requestBuilder, resolve, reject) => {
  let url = requestBuilder(l, viewResolution, coordinate, crs, 'text/plain');
  fetch(url)
  .then(res => res.text())
  .then(text => {
    resolve(text);
...
const parseFeatureInfo = async (l, viewResolution, coordinate, crs, cb) => {
  const callFormat = (format) => {
    return new Promise((resolve, reject) => {
      let requestBuilder = l.getSource() instanceof OlSourceWMTS ? buildFeatureInfoWMTSUrl : buildFeatureInfoWMSUrl;
      if (/gml/ig.test(format)) {
        getFeatureInfoGML(l, viewResolution, coordinate, crs, format, requestBuilder, resolve, reject);
      } else if (/xml/ig.test(format)) {
        getFeatureInfoXML(l, viewResolution, coordinate, crs, requestBuilder, resolve, reject);
      }
      else if (/geojson/ig.test(format)) {
        getFeatureInfoGeoJSON(l, viewResolution, coordinate, crs, requestBuilder, resolve, reject);
      }
      else if (/json/ig.test(format)) {
        getFeatureInfoJSON(l, viewResolution, coordinate, crs, requestBuilder, resolve, reject);
      }
      else if (/html/ig.test(format)) {
        getFeatureInfoHtml(l, viewResolution, coordinate, crs, requestBuilder, resolve, reject);
      }
      else if (/plain/ig.test(format)) {
        getFeatureInfoText(l, viewResolution, coordinate, crs, requestBuilder, resolve, reject);
      }
      else reject();
    });
  }
...
const buildFeatureInfoWMSUrl = (layer, viewResolution, coordinate, crs, format) => {  
...
const buildFeatureInfoWMTSUrl = (layer, viewResolution, coordinate, crs, format) => {
