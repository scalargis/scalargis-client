...
export default function PrintPanel4(props) {
  ...
  const { control, printGroup, printLayer, printDetails, finalPrints, setFinalPrints, fields, geometries, config, actions } = props;
  ...
  function mergePrintFiles(prints) {
    ...
    let url = API_URL + '/api/v2/app/viewer/'+ viewer.id + '/print/merge';
  ...
  useEffect(() => {
    ...
    const promises = finalPrints.map(async (print) => {
      ...
      const url = API_URL + '/api/v2/app/viewer/' + viewer.id + '/print/' + print.code + '/generate';
